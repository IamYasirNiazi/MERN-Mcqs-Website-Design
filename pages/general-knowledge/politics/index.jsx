...
const BasicGeneralKnowledge = ({mcqs}) => {
  ...
  useEffect(() => {
    if (page) {
      if(1<= page && page <=Math.ceil(mcqs.length/pageSize)){
        setCurrentPage(page)
        router.push(`/general-knowledge/politics?page=${parseInt(page)}`);
        console.log(page)
      }
    }
  }, [page]);
  ...
  function nextPage(event){
      ...
      router.push(`/general-knowledge/politics?page=${parseInt(currentPage) + 1}`)
  }
...
export async function getServerSideProps(){
  ...
  const res = await fetch(`${server}/api/politics`, {
