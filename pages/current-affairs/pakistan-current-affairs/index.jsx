...
const BasicGeneralKnowledge = ({mcqs}) => {
  ...
  useEffect(() => {
    if (page) {
      if(1<= page && page <=Math.ceil(mcqs.length/pageSize)){
        setCurrentPage(page)
        router.push(`/current-affairs/pakistan-current-affairs?page=${parseInt(page)}`);
        console.log(page)
      }
    }
  }, [page]);
  ...
  function nextPage(event){
      ...
      router.push(`/current-affairs/pakistan-current-affairs?page=${parseInt(currentPage) + 1}`)
  }
